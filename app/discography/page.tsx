...
export default function DiscographyPage() {
  ...
  // MENGOLAH DATA
  const { combinedDiscs, totalUniqueSongs } = useMemo(() => {
    ...
    const combined = [...albums, ...singles].sort(
      (a, b) =>
        new Date(b.release_date).getTime() - new Date(a.release_date).getTime(),
    );

    // Hitung Lagu Unik (Berdasarkan Judul Track)
    const uniqueSongs = new Set();
    combined.forEach((disc) => {
      disc.trackList.forEach((track) => uniqueSongs.add(track.title.trim()));
    });

    return { combinedDiscs: combined, totalUniqueSongs: uniqueSongs.size };
  }, []);
              ...
              <div className="relative w-full md:w-40 aspect-square bg-slate-100 rounded-lg flex-shrink-0 border border-slate-200 flex flex-col items-center justify-center text-slate-300 overflow-hidden group">
                {disc.image_path ? (
                  ...
                  <>
                    ...
                    <span className="text-[10px] mt-2">Cover Unavailable</span>
                  </>
                )}
              ...
              <div className="flex-1">
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  ...
                  <span className="text-xs font-medium text-gray-400 ml-auto">
                    Rilis:{" "}
                    {new Date(disc.release_date).toLocaleDateString("id-ID")}
                  </span>
                </div>
